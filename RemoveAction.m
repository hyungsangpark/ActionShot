function newImage = RemoveAction(imageArray)

% This function receives 1xn 1D cell array, with 3D image arrays in each
% cell, and creates a resultant image by finding the median rgb values
% between the values among the given images.
%
% This creates an image where differences between the images are removed in
% the resultant image.
%
...
for i = 1:rows
    ...
    for j = 1:cols
        ...
        for n = 1:length(imageArray)
            ...
            [newImage(i,j,1), newImage(i,j,2), newImage(i,j,3)] = MedianPixel(medianArray);
