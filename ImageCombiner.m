...
response = 'i'; % for testing purpose you may like to hard code the response to m or i here
while ~(strncmp(response,'m',1) || strncmp(response,'i',1))
    disp('What would you like to process?');
    resporesponsense = lower(input('Enter m for movie frames or i for images from a directory:','s'));
end
